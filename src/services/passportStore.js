...
export async function loadMyPassport(){
  ...
  if(passport?.id){const [opportunityResult,needResult]=await Promise.all([
    ...
    supabase.from("atlas_needs").select("id,group_key,item_key,quantity,unit,needed_from,needed_until,status,received_at,created_at,updated_at").eq("passport_id",passport.id).eq("owner_id",user.id).order("created_at",{ascending:false})]);
...
export async function addMyNeed(passportId,{groupKey,itemKey,unit,quantity,neededFrom,neededUntil}){const user=await ensureAtlasSession();const amount=Number(quantity);if(!passportId)throw fail("passport-required");if(!Number.isFinite(amount)||amount<=0)throw fail("quantity-invalid");if(!neededFrom||!neededUntil||neededUntil<neededFrom)throw fail("date-range-invalid");const {data,error}=await supabase.from("atlas_needs").insert({passport_id:passportId,owner_id:user.id,group_key:String(groupKey||"vegetables"),item_key:String(itemKey||"tomatoes"),quantity:amount,unit:String(unit||"шт").trim().slice(0,12),needed_from:neededFrom,needed_until:neededUntil,status:"not_received"}).select("id,group_key,item_key,quantity,unit,needed_from,needed_until,status,received_at,created_at,updated_at").single();if(error)throw error;return data}
