...
export async function loadMyPassport(){
  ...
  const {data:passport,error:passportError}=await supabase.from("atlas_passports").select("id,slug,display_name,entity_type,profession,skills,city,created_at,updated_at").eq("owner_id",user.id).maybeSingle();if(passportError)throw passportError;
...
export async function saveMyPassport({displayName,entityType="person",profession,skills,city,contact}){
  const user=await ensureAtlasSession();const cleanName=String(displayName||"").trim();const cleanEntityType=entityType==="company"?"company":"person";const cleanProfession=String(profession||"").trim().slice(0,240);const cleanSkills=String(skills||"").trim().slice(0,2000);const cleanCity=String(city||"").trim();const cleanContact=String(contact||"").trim();if(!cleanName)throw fail("display-name-required");if(!cleanContact)throw fail("contact-required");
  ...
  if(existing?.id){const {data,error}=await supabase.from("atlas_passports").update({display_name:cleanName,entity_type:cleanEntityType,profession:cleanProfession,skills:cleanSkills,city:cleanCity,updated_at:new Date().toISOString()}).eq("id",existing.id).eq("owner_id",user.id).select("id,slug,display_name,entity_type,profession,skills,city,created_at,updated_at").single();if(error)throw error;passport=data}else{const {data,error}=await supabase.from("atlas_passports").insert({owner_id:user.id,slug:createSlug(cleanName,user.id),display_name:cleanName,entity_type:cleanEntityType,profession:cleanProfession,skills:cleanSkills,city:cleanCity}).select("id,slug,display_name,profession,skills,city,created_at,updated_at").single();if(error)throw error;passport=data}
