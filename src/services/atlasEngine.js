...
import {searchAtlasPassports} from "./opportunitySearch.js";
...
/** Виконує повний opportunity-first цикл із нормалізованим статусом джерела. */
export async function buildOpportunityFirstSolution(goal,location="",lang="uk",forcedMode=null,client=undefined){
  const decision=buildDecisionSolution(goal,location,lang,forcedMode);
  const passportSearch=await searchAtlasPassports(decision.goal,location,client);
  return {...decision,passportSearch};
}
