...
export default function PageNavigation({title,t,showForward=false,showProfile=false,onNewRequest}){
  ...
  return <nav className="pageNavigation" aria-label={t.pageNavigation}><strong>{title}</strong><div><button className="navBack" onClick={()=>navigate("/")}><ArrowLeft/>{t.back}</button>{showForward&&<button className="navForward" onClick={()=>navigate("/solution")}><ArrowRight/>{t.forward}</button>}<button className="navHome" onClick={()=>navigate("/")}><Home/>{t.home}</button><button className="navNew" onClick={onNewRequest}><Plus/>{t.newRequest}</button>{showProfile&&<button className="navProfile" onClick={()=>navigate("/passport")}><UserRound/>{t.profile}</button>}</div></nav>;
}
