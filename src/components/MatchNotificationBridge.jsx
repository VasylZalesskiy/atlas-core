...
export default function MatchNotificationBridge({lang="uk"}){
  ...
  return <>
    ...
    {showToast&&<div className="atlasNotifyToast" role="status"><button className="atlasNotifyToastMain" onClick={()=>openItem(latest)}><strong>{latest.title||(uk?"Нове повідомлення Atlas":"New Atlas notification")}</strong><small>{latest.body||(uk?"Є нове повідомлення":"You have a new notification")}</small></button><button className="atlasNotifyToastClose" onClick={()=>setDismissedToastId(latest.id)} aria-label={uk?"Закрити":"Close"}><X size={16}/></button></div>}
    {showPanel&&<div className="atlasNotifyPanel">
      ...
      <div className="atlasNotifySetting"><div className="atlasNotifySettingText"><strong>{uk?"Звук повідомлень":"Notification sound"}</strong><small>{uk?"Короткий сигнал, коли в Atlas з’являється нове повідомлення або пропозиція.":"A short sound when a new Atlas message or proposal arrives."}</small></div><button className={`atlasSoundToggle ${sound?"on":"off"}`} onClick={toggleSound}>{sound?<Volume2 size={18}/>:<VolumeX size={18}/>}<span>{sound?(uk?"Увімкнено":"On"):(uk?"Вимкнено":"Off")}</span></button></div>
