...
export default function MatchNotificationBridge({lang="uk"}){
  ...
  async function refresh({silent=false}={}){
    try{
      ...
      try{window.dispatchEvent(new CustomEvent("atlas:notifications",{detail:{unread:unread.length}}));}catch{}
  ...
  async function enablePush(){
    if(pushBusy)return;setPushBusy(true);setPushState("");
    try{
      const result=await enablePushNotifications();
      if(result.status==="granted"){setPushEnabled(true);setPushState(uk?"Готово: Atlas може показувати повідомлення навіть коли застосунок закритий.":"Done: Atlas can notify you even when the app is closed.");}
  ...
  async function openItem(item){
    ...
    await refresh({silent:true});setShowPanel(false);setDismissedToastId(item.id);
  ...
  return <>
    ...
    {showToast&&<div className="atlasNotifyToast" role="status"><button className="atlasNotifyToastMain" onClick={()=>openItem(latest)}><strong>{latest.title||(uk?"Atlas знайшов збіг":"Atlas found a match")}</strong><small>{latest.body||(uk?"Є нове повідомлення":"You have a new notification")}</small></button><button className="atlasNotifyToastClose" onClick={()=>setDismissedToastId(latest.id)} aria-label={uk?"Закрити":"Close"}><X size={16}/></button></div>}
    {showPanel&&<div className="atlasNotifyPanel">
      ...
      <div className="atlasNotifySetting"><div className="atlasNotifySettingText"><strong>{uk?"Сповіщення на телефоні":"Phone notifications"}</strong><small>{uk?"Показуються навіть коли Atlas закритий. На ярлику буде цифра з кількістю нових повідомлень.":"Shown even when Atlas is closed. The app icon shows the number of new alerts."}</small></div>{pushEnabled?<span className="atlasSettingStatus on">{uk?"Увімкнено":"On"}</span>:<button className="atlasSettingAction" onClick={enablePush} disabled={pushBusy}>{pushBusy?(uk?"Підключаю…":"Enabling…"):(uk?"Підключити":"Enable")}</button>}</div>
      <div className="atlasNotifySetting"><div className="atlasNotifySettingText"><strong>{uk?"Звук нової потреби":"New-need sound"}</strong><small>{uk?"Короткий сигнал, коли Atlas знаходить нову потребу або збіг для вашої можливості.":"A short sound when Atlas finds a new need or match for your opportunity."}</small></div><button className={`atlasSoundToggle ${sound?"on":"off"}`} onClick={toggleSound}>{sound?<Volume2 size={18}/>:<VolumeX size={18}/>}<span>{sound?(uk?"Увімкнено":"On"):(uk?"Вимкнено":"Off")}</span></button></div>
