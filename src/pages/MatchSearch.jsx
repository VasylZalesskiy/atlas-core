    ...
    <section className="solutionFlows">
      ...
      {activeFlows.map(flow=>{
        ...
        return <article className={`solutionFlowCard ${closed?"flowClosed":""}`} key={flow.id}>
          <div className="solutionFlowTop"><div><span className="matchFoundBadge">{flow.role==="provider"?(uk?"Я НАДАЮ":"I PROVIDE"):(uk?"МОЯ ПОТРЕБА":"MY NEED")}</span><strong>{title}</strong>{needText&&<small>{uk?"Потреба":"Need"}: {needText}</small>}{counterpart?.display_name&&<div className="solutionCounterpart"><span>{flow.role==="provider"?(uk?"КОМУ НАДАЮ":"RECIPIENT"):(uk?"ХТО ПРОПОНУЄ":"PROVIDER")}</span><b>{counterpart.display_name}</b><em><MapPin size={14}/>{counterpart.city||(uk?"Місто не вказано":"City not specified")}</em></div>}</div><b className={`flowStatus flow-${flow.status}`}>{flowStatus(flow,uk)}</b></div>
    ...
    <section className="matchResults">
      ...
      {mode==="need"&&results.map(item=>{
        ...
        return <article className="matchResultCard" key={`${item.slug}-${item.opportunity_id||item.headline}`}>
          <div className="matchResultCopy"><span className="matchFoundBadge">{uk?"МОЖЛИВІСТЬ":"OPPORTUNITY"}</span><strong>{item.headline||item.name}</strong><div className="matchProvider"><span className="matchProviderLabel">{uk?"ХТО ПРОПОНУЄ":"PROVIDER"}</span><b>{item.name||(uk?"Користувач Atlas":"Atlas user")}</b>{item.city&&<span className="matchProviderCity"><MapPin size={15}/>{item.city}</span>}</div>{existing&&<b className={`resultFlowStatus flow-${existing.status}`}>{resultStatus(existing,uk)}</b>}</div>
