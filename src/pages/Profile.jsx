...
import {ArrowLeft,Camera,Check,ChevronRight,Copy,HandHeart,HeartHandshake,Inbox,LayoutGrid,MessagesSquare,Package,Pause,Pencil,Play,Plus,Share2,Sparkles,Tag,Trash2,X} from "lucide-react";
import {Link} from "react-router-dom";
import {addMyOpportunity,deleteMyOpportunity,loadIncomingRequests,loadMyPassport,loginAtlasAccount,logoutAtlasAccount,opportunityGroups,registerAtlasAccount,respondToPassportRequest,saveMyPassport,setMyOpportunityActive,setMyOpportunityCompleted,uploadOpportunityPhoto} from "../services/passportStore";
...
export default function Profile({lang="uk"}){
  ...
  async function completeOpportunity(item,completed){setBusyId(item.id);setError("");try{const updated=await setMyOpportunityCompleted(item,completed);setOpportunities(items=>items.map(value=>value.id===updated.id?updated:value));setNotice(completed?"Можливість виконано й перенесено в архів.":"Можливість повернуто до активних.")}catch(cause){setError(friendlyError(cause))}finally{setBusyId("")}}
