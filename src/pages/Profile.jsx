...
import {Camera,CheckCircle2,Copy,HeartHandshake,Inbox,Pause,Play,Plus,Share2,Trash2,X} from "lucide-react";
import {Link} from "react-router-dom";
import {addMyOpportunity,deleteMyOpportunity,loadIncomingRequests,loadMyPassport,loginAtlasAccount,logoutAtlasAccount,opportunityGroups,registerAtlasAccount,respondToPassportRequest,saveMyPassport,setMyOpportunityActive,uploadOpportunityPhoto} from "../services/passportStore";
...
const priceUnits=["кг","шт.","година","консультація","послуга","день","поїздка","комплект"];const saleUnits=["кг","шт.","т","л","комплект"];const currencySymbols={UAH:"грн",USD:"$",EUR:"€"};
const emptyEntry=()=>({group:"have",text:"",duration:"month",place:"",radiusValue:"",radiusUnit:"км",online:false,paymentType:"free",priceValue:"",priceUnit:"шт.",currency:"UAH",saleQuantity:"",saleUnit:"кг",validUntil:"",catalogGroupKey:"",catalogItemKey:"",catalogItemName:"",minimumQuantity:"",deliveryIncluded:false,photoUrl:"",photoLabel:"",photoTask:""});
function friendlyError(error){const text=String(error?.message||error||"");if(/anonymous|signups|disabled/i.test(text))return "У Supabase потрібно увімкнути анонімний вхід.";if(/atlas_requests|relation .*atlas_requests.*does not exist/i.test(text))return "Запити між користувачами ще не активовані.";if(/atlas_passports|atlas_opportunities|atlas_private_contacts|atlas_needs|atlas_need_groups|atlas_need_items|relation .* does not exist/i.test(text))return "База Паспортів ще не активована.";return text||"Не вдалося виконати дію."}
function cleanRequestMessage(value){return String(value||"").replace(/[\u200B-\u200D\u2060\u2062\u2063\uFEFF]*ATLAS_META:[^\r\n]*/gu,"").replace(/\n{3,}/g,"\n\n").trim()}
function OpportunityFields({value,onChange,textareaRef,compact=false,onPhoto,catalogGroups=[],catalogItems=[]}){
  ...
  function setOpportunityGroup(group){
    ...
    const unit=selectedItem?.unit||"кг";
...
<label><span>Підгрупа</span><select value={value.group} onChange={e=>setOpportunityGroup(e.target.value)}>{opportunityGroups.map(g=><option key={g.value} value={g.value}>{g.label}</option>)}</select></label>
...
export default function Profile({lang="uk"}){
  ...
  const [opportunities,setOpportunities]=useState([]),[requests,setRequests]=useState([]),[catalog,setCatalog]=useState({groups:[],items:[]});
  ...
  const activeCount=opportunities.filter(i=>i.is_active).length;
  ...
  async function reloadProfile(passportId=null,{showLoader=false}={}){
    ...
    setError("");
    try{
      const [d,catalogData]=await Promise.all([loadMyPassport(passportId),loadNeedCatalog().catch(()=>({groups:[],items:[]}))]);
  ...
  useEffect(()=>{
    let alive=true;
    Promise.all([loadMyPassport(),loadNeedCatalog().catch(()=>({groups:[],items:[]}))]).then(async ([d,catalogData])=>{
      if(!alive)return;
      setCatalog(catalogData);
  ...
  function scrollToAdd(){addRef.current?.scrollIntoView({behavior:"smooth",block:"start"});setTimeout(()=>textareaRef.current?.focus(),320)}
  ...
  async function addOpportunity(e){e.preventDefault();if(adding||photoBusy||!passport?.id)return;if(entry.group!=="sell"&&!entry.text.trim())return;if(entry.group==="sell"){if(!entry.catalogGroupKey||!entry.catalogItemKey){setError("Для продажу виберіть категорію та позицію зі списку.");return}if(!entry.priceValue||!entry.saleQuantity||!entry.validUntil){setError("Для продажу вкажіть ціну, кількість і термін дії.");return}if(entry.validUntil<new Date().toISOString().slice(0,10)){setError("Термін продажу не може бути в минулому.");return}}setError("");setAdding(true);try{const prepared=entry.group==="sell"?{...entry,text:entry.text.trim()||entry.catalogItemName}:entry;const a=await addMyOpportunity(passport.id,prepared);setOpportunities(x=>[a,...x]);setEntry(v=>({...emptyEntry(),group:v.group,...(v.group==="sell"?{paymentType:"paid",priceUnit:"кг",saleUnit:"кг"}:{})}));setNotice("Можливість додано.")}catch(cause){setError(friendlyError(cause))}finally{setAdding(false)}}
  ...
  return <main className="page appPage">
