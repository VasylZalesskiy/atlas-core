...
import {Camera,ChevronRight,Copy,HandHeart,HeartHandshake,Inbox,LayoutGrid,MessagesSquare,Package,Pause,Play,Plus,Share2,Sparkles,Tag,Trash2,X} from "lucide-react";
...
export default function Profile({lang="uk"}){
  ...
  const [showMobileEditor,setShowMobileEditor]=useState(false),[showMobileRecords,setShowMobileRecords]=useState(false),[showMoreGroups,setShowMoreGroups]=useState(false);
  ...
  async function selectPassport(id){if(!id)return;await reloadProfile(id,{showLoader:true});setEditingIdentity(false);setNotice("")}
  function createNewPassport(accountId){
    setPassport(null);setOpportunities([]);setRequests([]);setEntry(emptyEntry());setForm(blankForm());setEditingIdentity(true);setActiveAccountId(accountId||accounts[0]?.account_id||null);setError("");setNotice("Заповніть дані нової сторінки.");
    try{localStorage.removeItem("atlas-active-passport")}catch{}
  }

  function scrollToAdd(){setShowMobileEditor(true);requestAnimationFrame(()=>addRef.current?.scrollIntoView({behavior:"smooth",block:"start"}));setTimeout(()=>textareaRef.current?.focus(),320)}
  ...
  return <main className="page appPage">
    ...
    <section className="profileShell passportPage">
      ...
      {passport&&<div className="passportMobileMenu">
        ...
        <button type="button" className="passportMobileIdentity" onClick={()=>setEditingIdentity(true)}><span className="passportMobileAvatar">{form.displayName?.trim().charAt(0).toUpperCase()||"А"}</span><span><strong>{form.displayName}{form.city?` · ${form.city}`:""}</strong><small>Мій паспорт · змінити дані</small></span><ChevronRight size={20}/></button>
        ...
        <button type="button" className="passportMenuRow" aria-expanded={showMobileRecords} onClick={()=>{setShowMobileRecords(value=>!value);requestAnimationFrame(()=>document.getElementById("passport-records")?.scrollIntoView({behavior:"smooth",block:"nearest"}))}}><span className="menuIcon"><LayoutGrid size={20}/></span><span><strong>Мої записи</strong><small>{opportunities.length} можливостей</small></span><ChevronRight size={20}/></button>
      ...
      {passport&&<>
        ...
        <section className={"opportunityEditor "+(showMobileEditor?"mobileEditorOpen":"")} ref={addRef}><div className="sectionTitle"><div><span>НОВА МОЖЛИВІСТЬ</span><h2>Додати як є</h2></div><button type="button" className="mobileEditorClose" onClick={()=>setShowMobileEditor(false)} aria-label="Закрити форму"><X size={21}/></button></div><form onSubmit={addOpportunity}><OpportunityFields value={entry} onChange={setEntry} textareaRef={textareaRef} onPhoto={addPhoto} catalogGroups={catalog.groups} catalogItems={catalog.items} availableGroups={availableGroups}/><button className="primary addOpportunity" disabled={adding||photoBusy||(entry.group!=="sell"&&!entry.text.trim())}><Plus size={18}/>{photoBusy?"Додаю фото…":adding?"Додаю…":"Додати до Паспортa"}</button></form></section>
        <section id="passport-records" className={"opportunityGroups "+(showMobileRecords?"mobileRecordsOpen":"")}><div className="sectionTitle"><div><span>МОЇ МОЖЛИВОСТІ</span><h2>{opportunities.length} записів</h2></div></div>{opportunities.length===0&&<p className="groupEmpty">Ще немає записів. Оберіть одну з кнопок вище.</p>}{opportunityGroups.filter(g=>opportunities.some(i=>i.group===g.value)).map(g=>{const items=opportunities.filter(i=>i.group===g.value);return <details className="opportunityGroup" key={g.value} open><summary><span>{g.label}</span><b>{items.length}</b></summary><div className="opportunityList">{items.map(i=><article className={"opportunityCard "+(i.is_active?"":"paused")} key={i.id}>{i.photo_url&&<img className="opportunityCardPhoto" src={i.photo_url} alt={i.photo_label||i.text}/>}<div className="opportunityCopy"><p>{i.catalogItemName||i.text}</p>{i.catalogItemName&&i.text&&i.text!==i.catalogItemName&&<small>{i.text}</small>}{i.photo_label&&<small>{i.photo_label}</small>}<div className="opportunityMeta">{opportunityMeta(i).map(v=><span key={v}>{v}</span>)}<span>{i.visibility_scope==="groups"?"Лише групи":i.visibility_scope==="both"?"Atlas + групи":"Увесь Atlas"}</span></div></div><div className="recordActions"><button type="button" disabled={busyId===i.id} onClick={()=>toggleOpportunity(i)}>{i.is_active?<><Pause size={16}/>Призупинити</>:<><Play size={16}/>Активувати</>}</button><button className="deleteAction" type="button" onClick={()=>removeOpportunity(i.id)}><Trash2 size={16}/>Видалити</button></div></article>)}</div></details>})}</section>
