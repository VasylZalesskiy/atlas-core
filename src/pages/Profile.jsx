...
import {ArrowLeft,Camera,ChevronRight,Copy,HandHeart,HeartHandshake,Inbox,LayoutGrid,MessagesSquare,Package,Pause,Play,Plus,Share2,Sparkles,Tag,Trash2,X} from "lucide-react";
...
export default function Profile({lang="uk"}){
  ...
  const [mobileView,setMobileView]=useState("menu"),[showMoreGroups,setShowMoreGroups]=useState(false);
  ...
  async function selectPassport(id){if(!id)return;await reloadProfile(id,{showLoader:true});setEditingIdentity(false);setMobileView("menu");setNotice("")}
  function createNewPassport(accountId){
    setPassport(null);setOpportunities([]);setRequests([]);setEntry(emptyEntry());setForm(blankForm());setEditingIdentity(true);setMobileView("menu");setActiveAccountId(accountId||accounts[0]?.account_id||null);setError("");setNotice("Заповніть дані нової сторінки.");
    try{localStorage.removeItem("atlas-active-passport")}catch{}
  }

  function openMobileView(view){setMobileView(view);setError("");window.scrollTo({top:0,behavior:"auto"})}
  ...
  return <main className={`page appPage passportMobileView-${mobileView}`}>
    ...
    <section className="profileShell passportPage">
      ...
      {passport&&<div className="passportMobileMenu">
        ...
        <button type="button" className="passportMobileIdentity" onClick={()=>{setEditingIdentity(true);openMobileView("identity")}}><span className="passportMobileAvatar">{form.displayName?.trim().charAt(0).toUpperCase()||"А"}</span><span><strong>{form.displayName}{form.city?` · ${form.city}`:""}</strong><small>Мій паспорт · змінити дані</small></span><ChevronRight size={20}/></button>
        ...
        <button type="button" className="passportMenuRow" onClick={()=>openMobileView("records")}><span className="menuIcon"><LayoutGrid size={20}/></span><span><strong>Мої записи</strong><small>{opportunities.length} можливостей</small></span><ChevronRight size={20}/></button>
