...
import {Camera,Copy,HeartHandshake,Inbox,MessagesSquare,Pause,Play,Plus,Share2,Trash2,X} from "lucide-react";
  ...
  return <main className="page appPage">
    ...
    <section className="profileShell passportPage">
      ...
      {passport&&<>
        ...
        <section className="incomingRequests"><div className="sectionTitle"><h2><Inbox size={21}/>Вхідні{pendingRequests.length?(" · "+pendingRequests.length):""}</h2><Link className="needsShortcut" to="/messages"><MessagesSquare size={17}/>Усі повідомлення</Link></div>{pendingRequests.length===0?<p className="groupEmpty">Нових запитів немає.</p>:pendingRequests.map(i=><article key={i.id}><div><strong>{i.requester_name||"Користувач Atlas"}</strong><p>{cleanRequestMessage(i.message)}</p></div><div><Link className="primary" to={`/messages?thread=${i.id}`}><MessagesSquare size={16}/>Відповісти</Link><button className="secondary" onClick={()=>answerRequest(i.id,"declined")}><X size={16}/>Відхилити</button></div></article>)}</section>
      </>}
