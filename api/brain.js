...
function enforceSearchChannels(plan,query){
  ...
  if(service){
    ...
    plan.solution_scope="local_action";
    plan.needs_location=true;
    plan.external_searches=(plan.external_searches||[]).filter(item=>item?.source==="maps");
    if(!plan.external_searches.length){
      plan.external_searches=[{source:"maps",mode:"nearby",query:String(plan.goal||query).trim(),reason:"Find concrete local providers"}];
    }
    plan.solution_steps=(plan.solution_steps||[]).map(step=>({...step,nearby_relevant:true,nearby_query:String(step?.nearby_query||plan.goal||query).trim(),internet_relevant:false,internet_query:""}));
    return plan;
  }
