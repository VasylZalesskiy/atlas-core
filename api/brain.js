...
const schema={
  ...
  required:["understood","goal","intent","domain","solution_scope","urgency","needs_location","clarification","passport_search","solution_steps","external_searches","safety","result_strategy","answer"],
  properties:{
...
async function runDiagnostic(){
  try{
    ...
    const runner=openai.configured?runOpenAIResponse:runFreeAiResponse;
    ...
    return {
      api_call_ok:true,
      provider:openai.configured?"openai":"google-gemini-free-tier",
      model,
...
function isServiceQuery(query){
  return /шиномонтаж|ремонт|майстер|перукар|стомат|сервіс|послуг|монтаж|установ|налашту|мийк|евакуатор|нотаріус|адвокат|бухгалтер|готел|хостел|service|repair|installer|mechanic|barber|dentist|lawyer|accountant|hotel|hostel/iu.test(String(query||""));
}
function isMarketplaceQuery(query){
  return /куп|прод|придба|замов|товар|продукт|оренд(?:а|увати)|обмін|опт|гурт|постачаль|маркетплейс|оголош|buy|sell|order|product|rent|exchange|wholesale|supplier|marketplace|listing/iu.test(String(query||""));
}
function enforceSearchChannels(plan,query){
  if(!plan||typeof plan!=="object")return plan;
  const service=isServiceQuery(query);
  const informational=isInformationalQuery(query);
  const transaction=isMarketplaceQuery(query)&&!service;

  if(service){
    plan.domain="services";
    plan.solution_scope="information";
    plan.needs_location=true;
    plan.answer="";
    plan.external_searches=[{source:"web",mode:"standard",query:String(plan.goal||query).trim(),reason:"Find concrete local providers"}];
    plan.solution_steps=(plan.solution_steps||[]).map(step=>({...step,nearby_relevant:false,nearby_query:"",internet_relevant:true,internet_query:String(step?.internet_query||plan.goal||query).trim()}));
    return plan;
  }
  ...
  if(informational){
    plan.solution_scope="information";
    plan.answer="";
    plan.external_searches=(plan.external_searches||[]).map(item=>item?.source==="marketplace"?{...item,source:"web"}:item).filter(item=>item?.source!=="maps");
    if(!plan.external_searches.length){
      plan.external_searches=[{source:"web",mode:"standard",query:String(plan.goal||query).trim(),reason:"Find current external information"}];
    }
    plan.solution_steps=(plan.solution_steps||[]).map(step=>({...step,nearby_relevant:false,nearby_query:"",internet_relevant:true,internet_query:String(step?.internet_query||plan.goal||query).trim()}));
    return plan;
  }
  ...
  if(transaction){
    plan.solution_scope="transaction";
    plan.external_searches=(plan.external_searches||[]).filter(item=>item?.source==="marketplace");
    if(!plan.external_searches.length){
      plan.external_searches=[{source:"marketplace",mode:"standard",query:String(plan.goal||query).trim(),reason:"Find concrete products or listings"}];
    }
    plan.solution_steps=(plan.solution_steps||[]).map(step=>({...step,nearby_relevant:false,nearby_query:"",internet_relevant:true,internet_query:String(step?.internet_query||plan.goal||query).trim()}));
  }
  return plan;
...
export default async function handler(req,res){
  ...
  if(req.method==="GET"){
    ...
    const openai=await getOpenAIStatus();
    const base={status:"atlas-brain-endpoint-online",ai:openai.configured?openai:freeAi,openai,free_ai:freeAi};
  ...
  const fallback=(reason)=>{
    ...
    logAnalytics("atlas_brain_fallback",{reason:String(reason||"ai-unavailable").slice(0,120),duration_ms:Date.now()-startedAt});
  ...
  try{
    const openai=await getOpenAIStatus();
    const aiRunner=openai.configured?runOpenAIResponse:runFreeAiResponse;
    const {data,model}=await aiRunner({
      instructions:`${instructions}\n\n${solutionChainPolicy}\n\nReturn only the structured Atlas plan.`,
      input:JSON.stringify(context),
      maxOutputTokens:2600,
      timeoutMs:15000,
      ...(openai.configured?{schema,schemaName:"atlas_brain_plan"}:{})
    });
    ...
    return send(res,200,{plan,model,ai_status:openai.configured?"openai":"free-ai",location_context:locationContext});
  }catch(error){
